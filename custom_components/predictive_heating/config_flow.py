...
class PredictiveHeatingConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            for key in (CONF_HEATPUMP_ELECTRICITY_ENTITY, CONF_WEATHER_ENTITY):
                if user_input.get(key) and self.hass.states.get(user_input[key]) is None:
                    errors[key] = "entity_not_found"
            if not errors:
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                ...
                vol.Required(CONF_OUTDOOR_TEMP_ENTITY): selector.EntitySelector(
                    selector.EntitySelectorConfig(domain="sensor")),
                vol.Required(CONF_GAS_CONSUMPTION_ENTITY): selector.EntitySelector(
                    selector.EntitySelectorConfig(domain="sensor")),
                vol.Required(CONF_TOTAL_ELECTRICITY_ENTITY): selector.EntitySelector(
                    selector.EntitySelectorConfig(domain="sensor")),
    ...
    async def async_step_parameters(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is not None:
            cop_str = user_input.pop("cop_coefficients_str", f"{DEFAULT_COP_A},{DEFAULT_COP_B}")
            try:
                user_input[CONF_COP_COEFFICIENTS] = [float(x.strip()) for x in cop_str.split(",")][:2]
            except (ValueError, IndexError):
                user_input[CONF_COP_COEFFICIENTS] = [DEFAULT_COP_A, DEFAULT_COP_B]
            self._data.update(user_input)
        ...
        return self.async_show_form(
            step_id="parameters",
            data_schema=vol.Schema({
                vol.Required(CONF_HEATING_HOT_WATER_FRACTION, default=DEFAULT_HEATING_HOT_WATER_FRACTION):
                    selector.NumberSelector(selector.NumberSelectorConfig(min=0.0, max=1.0, step=0.05, mode="slider")),
                vol.Required(CONF_GAS_EFFICIENCY, default=DEFAULT_GAS_EFFICIENCY):
                    selector.NumberSelector(selector.NumberSelectorConfig(min=0.5, max=1.0, step=0.05, mode="slider")),
                vol.Required("cop_coefficients_str", default=f"{DEFAULT_COP_A},{DEFAULT_COP_B}"):
                    selector.TextSelector(),
                vol.Required(CONF_OUTDOOR_ELECTRIC_LOADS_W, default=DEFAULT_OUTDOOR_ELECTRIC_LOADS_W):
                    selector.NumberSelector(selector.NumberSelectorConfig(min=0, max=50000, step=100, unit_of_measurement="W")),
                vol.Required(CONF_GAS_PRICE, default=DEFAULT_GAS_PRICE):
                    selector.NumberSelector(selector.NumberSelectorConfig(min=0.0, max=5.0, step=0.01, unit_of_measurement="€/m³")),
                vol.Required(CONF_TRAINING_INTERVAL_DAYS, default=DEFAULT_TRAINING_INTERVAL_DAYS):
                    selector.NumberSelector(selector.NumberSelectorConfig(min=1, max=30, step=1)),
                vol.Required(CONF_TRAINING_WINDOW_DAYS, default=DEFAULT_TRAINING_WINDOW_DAYS):
                    selector.NumberSelector(selector.NumberSelectorConfig(min=7, max=90, step=1)),
                vol.Required(CONF_PREDICTION_HORIZON_HOURS, default=DEFAULT_PREDICTION_HORIZON_HOURS):
                    selector.NumberSelector(selector.NumberSelectorConfig(min=1, max=48, step=1)),
                vol.Required(CONF_OPTIMIZATION_TIMESTEP_MIN, default=DEFAULT_OPTIMIZATION_TIMESTEP_MIN):
...
class PredictiveHeatingOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        if user_input is not None:
            cop_str = user_input.pop("cop_coefficients_str", "")
            if cop_str:
                try:
                    user_input[CONF_COP_COEFFICIENTS] = [float(x.strip()) for x in cop_str.split(",")][:2]
                except (ValueError, IndexError):
